...
function initDatabase() {
  return new Promise((resolve, reject) => {
    // Define o caminho do banco de dados (funciona em desenvolvimento e produção)
    ...
    db = new sqlite3.Database(dbPath, (err) => {
      if (err) {
        ...
        console.log('Conectado ao banco de dados SQLite em:', dbPath);
         ...
         db.serialize(() => {
          // Tabela de Laudos (COMPLETA E CORRETA)
          db.run(`
          ...
          // --- NOVAS TABELAS PARA GERENCIAR DOENÇAS ---
          db.run(`
...
// --- NOVAS FUNÇÕES PARA GERENCIAR DOENÇAS E TESTES ---
  ...
  return new Promise((resolve, reject) => {
    const sql = `
      SELECT 
        ...
        t.test_id
      FROM doencas d
      LEFT JOIN testes t ON d.id = t.doenca_id
